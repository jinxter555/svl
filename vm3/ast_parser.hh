...
#ifndef YY_YY_AST_PARSER_HH_INCLUDED
# define YY_YY_AST_PARSER_HH_INCLUDED
// "%code requires" blocks.
#line 22 "ast_grammar.y"
...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // iexp
      char dummy4[sizeof (std::unique_ptr<NumberExpr<int>>)];
    };
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_iexp: // iexp
        value.move< std::unique_ptr<NumberExpr<int>> > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, std::unique_ptr<NumberExpr<int>>&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const std::unique_ptr<NumberExpr<int>>& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      ...
      case symbol_kind::S_iexp: // iexp
        value.template destroy< std::unique_ptr<NumberExpr<int>> > ();
        break;
      ...
      {
        return AstParser::symbol_name (this->kind ());
      }
#endif // #if YYDEBUG || 0
    ...
    AstParser (AstScanner* scanner_yyarg);
    virtual ~AstParser ();
...
#if YY_CPLUSPLUS < 201103L
    /// Non copyable.
    AstParser (const AstParser&);
    /// Non copyable.
    AstParser& operator= (const AstParser&);
#endif
...
#line 1456 "ast_parser.hh"




#endif // !YY_YY_AST_PARSER_HH_INCLUDED
