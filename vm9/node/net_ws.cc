...
vector<SocketListener::sl_uptr>  SocketListener::sl_objects={};
vector<SocketSession::ss_uptr>  SocketSession::ss_objects={};
vector<SocketClient::sc_uptr>  SocketClient::sc_objects={};
...
Node::Integer SocketSession::create_and_register(tcp::socket&& socket) {
  auto sptr = make_unique<SocketSession>(move(socket));
  sptr->accept_handshake();
  Node::Integer id = ss_objects.size();
  ss_objects.push_back(move(sptr));
  return id;
}
...
Node::Integer SocketListener::create_and_register(const string& address,  uint16_t port) {
  auto sptr = make_unique<SocketListener>(address, port);
  Node::Integer id =  sl_objects.size();
  sl_objects.push_back(move(sptr));
  return id;
}
...
Node::Integer SocketClient::create_and_register() {
  auto sptr = make_unique<SocketClient>();
  Node::Integer id =  sc_objects.size();
  sc_objects.push_back(move(sptr));
  return id;
}
