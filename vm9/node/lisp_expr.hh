...
private:
 ...
 // static const vector<string> lisp_process; 
  ...
  atom_pid, atom_ppid, atom_queue, atom_swap, atom_send, atom_receive, atom_worker, atom_inbox, atom_ipc, atom_print, atom_self, atom_eval, atom_wait,
  atom_websocket, atom_server, atom_client
  ;
...
public: 
  ...
  Node::OpStatus var_attach_gscope(Node&process, Node&scope, const Node::Vector& var_list, const string& modname,  size_t start=0);
  ...
  Node::OpStatus assign_attach_gscope(Node&process, Node&scope, const string& identifier, unique_ptr<Node>value_ptr) ; //  assign a value 
...
//  Node::OpStatus call_object(Node& process, const Node::Map & obj, const Node::Vector& args); // call object with Node::Map as primitive type
  ...
  Node::OpStatus build_parsed_alias(Node::List& list); // ((k1 v1)(k2 v2)...)
  ...
  Node::ControlFlow handle_cf_object(Node&process, Node::Vector &result_list, const Node::Map& object, const Node::Vector &args); // handles (return ... :ok )
