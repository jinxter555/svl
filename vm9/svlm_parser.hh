  ...
  private:
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_kv_pair: // kv_pair
        value.move< tuple<string, astnode_u_ptr> > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, astnode_u_ptr&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const astnode_u_ptr& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, tuple<string, astnode_u_ptr>&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const tuple<string, astnode_u_ptr>& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      ...
      case symbol_kind::S_kv_pair: // kv_pair
        value.template destroy< tuple<string, astnode_u_ptr> > ();
        break;
