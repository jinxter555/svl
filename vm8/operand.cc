...
//--------------------------------------
astexpr_u_ptr& Operand::operator[] (const Operand& k) {
  return const_cast<astexpr_u_ptr&>(as_const(*this)[k]); 
}
const astexpr_u_ptr& Operand::operator[] (const Operand &k) const {
  auto &ptr = get_u_ptr();
  if(ptr==nullptr) return nil_ast_ptr;
  return (*ptr)[k];
}
//-----------------------------------------------------------------------
...
AstExpr *Operand::get_raw_ptr(s_integer i) {
...
vector<string> Operand::_get_keys() {
  auto &node = get_u_ptr();
