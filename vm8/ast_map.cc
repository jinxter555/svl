...
Operand& AstMap::getv()  {
  //if(this==nullptr) return nil_operand;
  return myself;
}
...
//--------------------------------------
astexpr_u_ptr& AstMap::operator[] (const Operand& k) {
  return const_cast<astexpr_u_ptr&>(as_const(*this)[k._get_str()]); 
}
const astexpr_u_ptr& AstMap::operator[] (const Operand &k) const {
  const string i = k._get_str();
  if(!has_key(i)) return nil_ast_ptr;
  return map_.at(i); 
}
...
//------------------------------------- 
Operand AstMap::opfunc(const Operand& other, AstOpCode op) {
  return  Operand();
}
//------------------------------------- 
...
Operand& AstMap::get_branch(const vector<string> &keys) {
  ...
  for(int i=0; i<s-1; i++) {
    k = keys[i];  //cout << "keys[" << i<< "]: " << keys[i] <<  " "  << "\n"; 
    // if(curr!=nullptr) cout << "keys[" << i<< "]: " << keys[i] <<  " "  << curr->get_type() << "\n";
    next = (AstMap*) curr->get_raw_ptr(k);
    if(curr==nullptr || curr->type_ != OperandType::map_t)  { 
      return nil_operand;
    }
    curr = next;
  }
  /*
  cout << "last key: " <<  keys.back() << "\n";
  auto &v = curr->getv(keys.back());
  cout << "curr v: " <<  v << "\n";
*/
...
vector<string> AstMap::_get_keys() {
